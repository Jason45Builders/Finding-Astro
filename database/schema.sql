...
CREATE TABLE IF NOT EXISTS animals (
  ...
  visibility animal_visibility NOT NULL DEFAULT 'private',
  visibility_reason TEXT,
  visibility_expires_at TIMESTAMPTZ,
  visibility_changed_by UUID REFERENCES users(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS welfare_orgs (
  ...
  upi_verified_by UUID REFERENCES users(id),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Welfare org admins (minimum membership for payment management)
...
-- Welfare group direct UPI donations (manual verification)
...
-- Welfare payment event audit trail
