...
const ENDPOINT = "http://localhost:3000/api/v1/animals";
const TOKEN = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIwMDAwMDAwMC0wMDAwLTAwMDAtMDAwMC0wMDAwMDAwMDAwMDEiLCJlbWFpbCI6Imd1ZXN0QHRlc3QubG9jYWwiLCJyb2xlIjoiZ3Vlc3Rfc3lzdGVtIiwiZXhwIjoxNzg1MzEzMDg1fQ.00iBCOgrxjJgO2sEQzn_28Bb-nCg-tfrRu_b99wNnjE";
...
(async () => {
  ...
  const authErrorCount = statuses.filter(s => s === 401).length;
  ...
  console.log(`  Success (200): ${successCount}/${CONCURRENT_REQUESTS}`);
  console.log(`  Server errors (500): ${errorCount}/${CONCURRENT_REQUESTS}`);
  console.log(`  Auth errors (401): ${authErrorCount}/${CONCURRENT_REQUESTS}`);
  ...
  if (successCount === CONCURRENT_REQUESTS) {
    ...
    console.log("\n✗ Server errors detected — check for WebSocket or other crashes");
  } else {
    console.log("\n? Mixed results");
  }
})().catch(console.error);
